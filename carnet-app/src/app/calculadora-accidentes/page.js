...
export default function CalculadoraAccidentes() {
  ...
  // Sincronizar fecha según la sede seleccionada (en Pereira se fija 08-01-2020 o su valor manual guardado)
  useEffect(() => {
    if (isPereira) {
      const saved = localStorage.getItem("last_accident_date_pereira");
      setLastAccidentDate(saved || FIXED_PEREIRA_DATE);
    } else {
      const cityKey = (selectedCity || "general").toLowerCase();
      const saved = localStorage.getItem(`last_accident_date_${cityKey}`);
      if (saved) {
        setLastAccidentDate(saved);
      } else {
        const defaultDate = new Date();
        defaultDate.setDate(defaultDate.getDate() - 21);
  ...
  const handleAccidentDateChange = (newDate) => {
    ...
    if (isPereira) {
      // Guardar de forma manual y persistente para Pereira
  ...
  return (
    <div className="container" style={{ maxWidth: '1000px' }}>
      <header style={{ textAlign: 'center', marginBottom: '2.5rem', marginTop: '1.5rem' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', backgroundColor: '#00205b', color: '#fcd116', padding: '0.35rem 1rem', borderRadius: '20px', fontSize: '0.85rem', fontWeight: '800', marginBottom: '0.75rem', textTransform: 'uppercase' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem' }}>
          {isPereira 
            ? "CD Pereira • Fecha fija de último accidente: 08-01-2020 (solo actualizable manualmente)."
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2rem', marginTop: '2.5rem' }}>
        ...
        <div className="glass-panel" style={{ width: '100%', maxWidth: '650px', padding: '2rem', textAlign: 'center' }}>
          ...
          {isPereira && (
            <div style={{ backgroundColor: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', padding: '0.85rem 1.25rem', marginBottom: '1.5rem', textAlign: 'left', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', color: '#166534', fontSize: '0.88rem' }}>
                <span style={{ fontSize: '1.2rem' }}>🔒</span>
                <div>
                  <strong>CD Pereira:</strong> Fecha base fijada en <strong>08-01-2020</strong>.
                  <div style={{ fontSize: '0.78rem', color: '#15803d' }}>Esta fecha no cambia automáticamente; solo tú puedes modificarla manualmente.</div>
                </div>
              </div>
              {lastAccidentDate !== FIXED_PEREIRA_DATE && (
                <button
                  onClick={() => handleAccidentDateChange(FIXED_PEREIRA_DATE)}
                  style={{ background: '#00205b', color: '#fcd116', border: 'none', padding: '0.4rem 0.8rem', borderRadius: '6px', fontSize: '0.78rem', fontWeight: '800', cursor: 'pointer' }}
                >
                  ↺ Volver a 08-01-2020
                </button>
              )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', margin: '0.25rem 0 0 0' }}>
              {isPereira 
                ? "Guardado manual persistente para CD Pereira."
