...
function readExcelFallback() {
  try {
    ...
    const candidates = [
      path.join(publicDir, 'Barrancabermeja', 'Inducciones', 'Inducción Glp.xlsx'),
      path.join(parentDir, 'Barrancabermeja', 'Inducciones', 'Inducción Glp.xlsx'),
      path.join(process.cwd(), 'Barrancabermeja', 'Inducciones', 'Inducción Glp.xlsx')
    ];
    ...
    return rawRows.map((r, idx) => {
      ...
      const fechaVencLic = formatExcelDate(r['Fecha de vencimiento de la licencia']);
      ...
      return {
        ...
        tipo_licencia: String(r['Tipo de licencia de conducción'] || 'B1').trim(),
        ...
        empresa: String(r['Empresa o persona natural bajo la cual esta afiliado'] || 'GLP').trim(),
        ...
        respuestas_evaluacion: {
          normas_seguridad: r['1. Seleccione la respuesta correcta con respecto a las normas de seguridad del cd'],
          limite_velocidad: r['2. El limite de velocidad dentro del cd es'],
          riesgo_glp: r['3. Cuá es el principal riesgo asociado a glp'],
          olor_gas: r['4. ¿Si durante el ingreso al cd detecta olor a gas en su vehículo, que debe hacer? '],
          bloqueo_glp: r['5. Cual es el proceso de bloqueo del vehículo de glp en el área de descargue de cilindros'],
    ...
    console.error('Error reading GLP excel fallback:', err);
    return [];
  }
}
...
export async function GET() {
  try {
    ...
    console.error('Error GET /api/inducciones/glp:', error);
...
export async function POST(request) {
  try {
    ...
    if (db) {
      const { data, error } = await db.from('induccion_glp').insert([newRecord]).select();
    ...
    console.error('Error POST /api/inducciones/glp:', error);
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
}
